...
public class LavaBullet : MonoBehaviour
{
    ...
    private void HitTarget() //for now just destroys itself
    {
        ...
        GameObject puddleInstance = (GameObject)Instantiate(lavaPool, transform.position + Vector3.down, Quaternion.LookRotation(Vector3.forward));
        Destroy(puddleInstance, 2f);
        Destroy(gameObject);
    }
