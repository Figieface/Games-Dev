...
public class CannonTowerRampingDamage : MonoBehaviour
{
    ...
    public float spinSpeed = 130;
    private GameObject oldTarget;
    private bool sameEnemy;
    ...
    private void UpdateTarget()
    {
        ...
        if (nearestEnemy == oldTarget) sameEnemy = true;
        else sameEnemy = false;
       
        if (nearestEnemy != null && shortestDist <= range) //if closest enemy is in range
        {
            target = nearestEnemy.transform;
            oldTarget = nearestEnemy;
        }
    ...
    private void Update()
    {
        ...
        wheelSpinner.transform.Rotate(Vector3.right * spinSpeed * Time.deltaTime, Space.Self);
    ...
    private void Shoot()
    {
        ...
        else if (sameEnemy && bullet.damage <= 60)
        {
            bullet.damage += 15;
            spinSpeed += 40;
        }
